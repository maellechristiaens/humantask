...
m = max(cellfun(@max, stimuli(:,13))); %store the number of conditions seen 
...
for i=1:length(stimuli) %for each trial
    ...
    if stimuli{i,11} == 4 %if it is a wrong touch during test1
        err_time(1, i) = 3; 
        err_cond(1, stimuli{i,13}) = err_cond(1, stimuli{i,13}) + 1; %add one to the count of test1 errors by condition (one of the first 3 elements of the matrix)
    elseif stimuli{i,11} == 5 %if it is a wrong touch during test2
        err_time(1, i) = 3;
        err_cond(1, m + stimuli{i,13}) = err_cond(1, m + stimuli{i,13}) + 1; %add one to the count of test2 errors by condition (one of the last 3 elements of the matrix)
