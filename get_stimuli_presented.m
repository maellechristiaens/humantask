...
for i=1:length(data) %for each row of the file (ie each trial)
    stimuli{i,14} = name; %in each row, write the subject's name 
...
for i = 1:nb_conditions %for the number of conditions 
    for j = change(i*2-1):change(i*2+1) %between the start and the end of the condition for the first test phase
        stimuli{j,13} = i; %write the condition number
    end
    for k = change(nb_conditions*2 + i*2-1):change(nb_conditions*2 + i*2+1) %between the start and the end of the condition for the second test phase
        stimuli{k,13} = i; %write the condition number
    end
end
